...
export function parseSource(xml) {
  ...
  // The coordinate space is the outermost element's box — the app / window / root
  // layout, i.e. the device screen — NOT the max over all elements. iOS reports
  // off-screen scrollable content (carousels, long scroll views) with bounds far
  // beyond the screen; using the max would shrink every overlay and break the
  // stage aspect ratio. The wrapper (<hierarchy> / <AppiumAUT>) has no bounds, so
  // the first drawable node in document order is the screen-covering root.
  const rootBox = nodes.find((n) => n.rect);
  const width = rootBox ? rootBox.rect.x2 : 0;
  const height = rootBox ? rootBox.rect.y2 : 0;
