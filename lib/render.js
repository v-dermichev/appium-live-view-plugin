...
export function buildLiveViewHtml(options = {}) {
  ...
  const isBox = (n) => n.rect && n.rect.w > 0 && n.rect.h > 0;
  ...
  const treeRows = nodes
    .map((n) => {
      ...
      return (
        ...
        (isBox(n) ? '' : ` <span class="lv-node-nobox" title="no bounds — not clickable on the screenshot">◌</span>`) +
        `</label>`
      );
    })
    .join('\n');
  ...
  ${
    nodes.length || xmlB64 || shot
      ? `<span class="lv-tools">${
          ...
          xmlB64
            ? `<a id="lv-xml" class="lv-btn" target="_blank" rel="noopener" title="Open the page source (XML). Ctrl/Cmd-click for a new tab; right-click to save">XML</a>`
            : ''
        }${
          shot
            ? `<a id="lv-img" class="lv-btn" target="_blank" rel="noopener" title="Open the screenshot. Ctrl/Cmd-click for a new tab; right-click to save">Image</a>`
            : ''
        }</span>`
      : ''
  }
...
(function(){
  ...
  // Give the XML/Image controls real (blob) hrefs, so they behave like links:
  // click, Ctrl/Cmd-click for a new tab, or right-click to save.
  function lvSetLinks(){
    var ax=document.getElementById("lv-xml");
    if(ax&&LV_XML){try{ax.href=URL.createObjectURL(new Blob([LV_XML],{type:"application/xml"}));}catch(e){}}
    var ai=document.getElementById("lv-img"),shot=document.querySelector(".lv-shot");
    if(ai&&shot){var blob=lvBlobFromDataUri(shot.getAttribute("src"));if(blob){try{ai.href=URL.createObjectURL(blob);}catch(e){}}}
  }
  lvSetLinks();
