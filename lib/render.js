...
.lv-loc.lv-copied{background:rgba(34,197,94,.16);outline-color:rgba(34,197,94,.6)}
...
.lv-loc.lv-copied .lv-loc-hint{opacity:1;color:#16a34a}
/* user-select:all -> one click selects the whole locator, so it can be copied
   with Ctrl/Cmd+C even inline in Allure, where scripts do not run. */
.lv-loc-value{display:block;margin-top:3px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:12px;word-break:break-all;user-select:all}
...
.lv-loc.lv-copied{background:rgba(34,197,94,.22)}
.lv-head input[type=search]{background:#3a3a3c;border-color:rgba(255,255,255,.2)}
}
</style>
${radios}
<div class="lv-head">
...
(function(){
  ...
  document.addEventListener("click",function(e){
    ...
    lvCopy(v).then(function(){
      ...
      li.__lvT=setTimeout(function(){li.classList.remove("lv-copied");if(hint){hint.textContent="click to copy";}},1200);
    }).catch(function(){
      // Clipboard unavailable — select the value so it can be copied manually.
      lvSelect(li);
