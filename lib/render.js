...
export function buildLiveViewHtml(options = {}) {
  ...
  const overlays = nodes
    ...
    .map((n) => {
      ...
      const z = n.depth + (onScreen(n) ? 1000 : 0);
      const style =
        `left:${pct(r.x1, extents.width)};top:${pct(r.y1, extents.height)};` +
        `width:${pct(r.w, extents.width)};height:${pct(r.h, extents.height)};z-index:${z}`;
      if (isBox(n)) {
  ...
  const selectionCss = nodes
    .map((n) => {
      ...
      if (isBox(n)) {
        ...
        css += `#lv-r-${n.index}:checked~.lv-main .lv-el-${n.index}{display:block;outline:2px dotted #e5484d;background:rgba(229,72,77,.1);z-index:7000}`;
      }
      return css;
    })
    .join('');
