...
.lv-btn{font:inherit;font-size:12px;padding:4px 10px;border:1px solid rgba(0,0,0,.2);border-radius:6px;background:#fff;color:inherit;cursor:pointer;white-space:nowrap}
...
<div class="lv-head">
  ...
  ${
    xmlB64 || shot
      ? `<span class="lv-tools">${
          xmlB64
            ? `<button type="button" id="lv-xml" class="lv-btn" title="Open the page source (XML) in a new tab">View XML</button>`
            : ''
        }${
          shot
            ? `<button type="button" id="lv-img" class="lv-btn" title="Download the screenshot">Save image</button>`
            : ''
        }</span>`
      : ''
  }
...
(function(){
  ...
  function lvDownload(blob,name){var u=URL.createObjectURL(blob),a=document.createElement("a");a.href=u;a.download=name;document.body.appendChild(a);a.click();a.remove();setTimeout(function(){URL.revokeObjectURL(u);},2000);}
  function lvOpenBlob(blob,fallbackName){var u=URL.createObjectURL(blob),w=null;try{w=window.open(u,"_blank");}catch(e){}if(!w){lvDownload(blob,fallbackName);}setTimeout(function(){URL.revokeObjectURL(u);},60000);}
  function lvWireTools(){
    var bx=document.getElementById("lv-xml");
    if(bx&&LV_XML){bx.addEventListener("click",function(){lvOpenBlob(new Blob([LV_XML],{type:"application/xml"}),"page-source.xml");});}
    var bi=document.getElementById("lv-img"),shot=document.querySelector(".lv-shot");
    if(bi&&shot){bi.addEventListener("click",function(){var blob=lvBlobFromDataUri(shot.getAttribute("src"));if(!blob)return;var ext=/svg/.test(blob.type)?"svg":(/(png|jpe?g|webp|gif)/.exec(blob.type)||[0,"png"])[1];lvDownload(blob,"screenshot."+ext);});}
  }
  lvWireTools();
  ...
  function lvClearHits(){var h=document.querySelectorAll(".lv-xhit");for(var i=0;i<h.length;i++){h[i].classList.remove("lv-xhit");}}
  function lvRunXpath(q){
    ...
    for(var i=0;i<n;i++){
      ...
      var ov=idx>=0?document.querySelector(".lv-el-"+idx):null;
